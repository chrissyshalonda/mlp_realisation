...
class BatchNormalization(Module):
    ...
    def compute_grad_input(self, input: np.ndarray, grad_output: np.ndarray) -> np.ndarray:
        ...
        B = input.shape[0]
        if self.affine:
            d_out = grad_output * self.weight
        else:
            d_out = grad_output

        if self.training:
          grad_input = (B * d_out - np.sum(d_out, axis=0) - \
            self.norm_input * np.sum(d_out * self.norm_input, axis=0)
          )
          grad_input *= (self.inv_sqrt_var / B)
        else:
          grad_input = d_out * self.inv_sqrt_var
...
class Sequential(Module):
    ...
    def compute_output(self, input: np.ndarray) -> np.ndarray:
        ...
        for layer in self.modules:
          input = layer.compute_output(input)
    ...
    def compute_grad_input(self, input: np.ndarray, grad_output: np.ndarray) -> np.ndarray:
        ...
        """
        # replace with your code ｀、ヽ｀、ヽ(ノ＞＜)ノ ヽ｀☂｀、ヽ
        indexed_modules = list(enumerate(self.modules))

        for i, layer in reversed(indexed_modules):
          if i > 0:
            current_input = self.modules[i-1].output
          else:
            current_input = input

          layer.update_grad_parameters(current_input, grad_output)
          grad_output = layer.compute_grad_input(current_input, grad_output)
